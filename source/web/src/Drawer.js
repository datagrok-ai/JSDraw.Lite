...
export class Drawer {
    drawFormula(surface, p, reversed, s, color, fontsize) {
        //I#11940
        if (reversed) {
            const c = s.charAt(0);
            if (c >= '0' && c <= '9')
        ...
        for (let i = 0; i < ss.length; ++i) {
            if (reversed) {
                if (ss[i].num != null) {
                    const r = this.drawWord(surface, rect, p, color, fontsize, ss[i].num, reversed, true);
                    if (rect.isEmpty())
                        rect = r;
                    else
                        rect.union(r);
    ...
    drawWord(surface, rect, p, color, fontsize, w, reversed, isnumber) {
        if (isnumber)
            fontsize /= 1.4;
    ...
    drawCurveArrow(surface, p1, p2, p1a, p2a, color, linewidth) {
    ...
    drawCurve(surface, p1, p2, deg, r, color, linewidth) {
    ...
    drawArrow(surface, p1, p2, color, linewidth, dottedline, arrowstyle) {
        if (arrowstyle == "dual") {
    ...
    drawArrowhead(surface, p1, p2, color, linewidth, arrowstyle) {
        if (arrowstyle == "solid") {
    ...
    drawArrowhead2(surface, p1, p2, color, linewidth, as) {
    ...
    drawDoubleArrow(surface, r, color, linewidth) {
        const m = linewidth * 3;
        const w = linewidth;
    ...
    drawLabel(surface, p, s, fontcolor, fontsize, opaque, align, offsetx, stroke) {
        const w = fontsize + 2;
        if (opaque) {
    ...
    drawText2(surface, p, s, fontcolor, fontsize, rotatedeg) {
        const w = fontsize + 2;
        let t = null;
        if (dojox.gfx.renderer == "canvas") {
    ...
    drawText(surface, p, s, fontcolor, fontsize, align, italic) {
        if (align == null)
        ...
        if (dojox.gfx.renderer == "canvas") {
            ...
            const font = { family: "Arial", size: (fontsize < this.kMinFontSize ? this.kMinFontSize : fontsize) /*Mac,Linux bug*/ + "px", weight: "normal" };
            if (italic)
                font.style = "italic";
    ...
    drawBasis(surface, p1, p2, color, linewidth) {
    ...
    drawCurves(surface, p1, p2, color, linewidth) {
    ...
    drawLine(surface, p1, p2, color, linewidth, dotline, cap) {
        if (linewidth == null)
            linewidth = 1;
    ...
    drawRect(surface, r, color, linewidth, radius, style) {
        if (r == null || r.isEmpty())
            return;
        const opts = { x: r.left, y: r.top, width: r.width, height: r.height };
        if (radius != null)
            opts.r = radius;
        if (style != null)
            opts.style = style;
    ...
    drawDShape(surface, r, color, linewidth, reverse) {
    ...
    drawDiamond(surface, r, color, linewidth) {
        const c = r.center();
        const points = [
            { x: c.x, y: r.top },
            { x: r.right(), y: c.y },
    ...
    drawHexgon(surface, r, color, linewidth) {
        const c = r.center();
    ...
    drawPentagon(surface, r, color, linewidth) {
        const c = r.center();
